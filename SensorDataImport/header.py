    ...
    def __init__(self, headerPacket=None):
        if(headerPacket is None):
            ...
            try:
                ...
                #self.unixTime_start = int(headerPacket[16]) | (int(headerPacket[15]) << 8) | (int(headerPacket[14]) << 16) | (int(headerPacket[13]) << 24);
            ...
            try:
                packedDateTime = int(headerPacket[20]) | (int(headerPacket[21]) << 8) | (int(headerPacket[22]) << 16) | (int(headerPacket[23]) << 24)
                #self.unixTime_stop = int(headerPacket[23]) | (int(headerPacket[22]) << 8) | (int(headerPacket[21]) << 16) | (int(headerPacket[20]) << 24);
                ...
                m = (packedDateTime >> 22) & 0x0F;
                y = (packedDateTime >> 26) & 0x3F;
                self.stop_timeStamp = datetime.datetime(hour=H, minute=M, second=S, day = d, month = m, year = 2000+y)
            except: # catch *all* exceptions
                print("Error while extracting timestamp! Using default timestamp: 00:00:00/01.01.2017")
                self.stop_timeStamp = datetime.datetime(hour=0, minute=0, second=0, day = 1, month = 1, year = 2017)
