...
class session:
    ...
    @classmethod
    def from_filePaths(cls, leftFootPath, rightFootPath,header, freeRTOS = 0):
    ...
    @classmethod
    def from_folderPath(cls, folderPath, header, freeRTOS = 0):
    ...
    def convertToDataFrame(self, session):
        ...
        dfLeft = pd.DataFrame(np.hstack((rtc,dataset.acc.data, dataset.gyro.data, dataset.pressure.data, baro,battery)))
        foot = 'L'
        dfLeft.columns = ['utc'+foot,'aX'+foot,'aY'+foot,'aZ'+foot, 'gX'+foot,'gY'+foot,'gZ'+foot, 'p1'+foot, 'p2'+foot,'p3' + foot,'b' + foot, 'bat' + foot ]
        
        dataset = session.rightFoot;
        baro = np.reshape(dataset.baro.data,(dataset.size,1))
        battery = np.reshape(dataset.battery.data,(dataset.size,1))
        rtc = np.reshape(dataset.rtc,(dataset.size,1))
        dfRight = pd.DataFrame(np.hstack((rtc,dataset.acc.data, dataset.gyro.data, dataset.pressure.data, baro,battery)))
        foot = 'R'
        dfRight.columns = ['utc'+foot,'aX'+foot,'aY'+foot,'aZ'+foot, 'gX'+foot,'gY'+foot,'gZ'+foot, 'p1'+foot, 'p2'+foot,'p3' + foot,'b' + foot, 'bat' + foot ]
